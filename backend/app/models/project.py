...
from sqlalchemy import Column, String, Numeric, Enum as SAEnum, DateTime, ForeignKey, Text
...
class ProjectInvestment(Base):
    ...
    domestic_entity_id = Column(UUID(as_uuid=True), ForeignKey("entities_domestic.entity_id"), nullable=True)
    overseas_entity_id = Column(UUID(as_uuid=True), ForeignKey("entities_overseas.entity_id"), nullable=True)
    ...
    currency = Column(SAEnum(InvestmentCurrency), default=InvestmentCurrency.USD, comment="币种")
    investment_path = Column(SAEnum(InvestmentPath), default=InvestmentPath.DIRECT, comment="投资架构")
    funding_source = Column(JSONB, nullable=True, comment="资金来源构成")
    purpose_description = Column(Text, nullable=True, comment="投资必要性说明")
    status = Column(SAEnum(ProjectStatus), default=ProjectStatus.PRE_REVIEW, comment="当前状态")
    ...
    ndrc_certificate_url = Column(String(500), nullable=True, comment="发改委备案通知书")
    mofcom_certificate_url = Column(String(500), nullable=True, comment="商务部投资证书")
    ...
    status_logs = relationship("ProjectStatusLog", back_populates="project", lazy="selectin")
...
class ProjectStatusLog(Base):
    ...
    project_id = Column(UUID(as_uuid=True), ForeignKey("projects_investment.project_id"), nullable=False)
